...
#Manual data
ManualData<-read.csv("data/trial1/wp.csv")
str(ManualData)

library(dplyr)
SE<-function(x){sd(x,na.rm=T)/sqrt(length(x))}
sum_WP<-d%>%
  mutate(datetime=as.POSIXct(strptime(datetime, "%m/%d/%Y %H:%M")), date=as.POSIXct(paste0(date," 00:00"), format="%m/%d/%Y %H:%M"))%>%
  group_by(date)%>%
...
library(ggplot2)
fig1<-ggplot()+
  geom_point(filter(auto_data, values > 0.01), aes(x=datetime, y=psy)) +
